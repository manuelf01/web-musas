...
|---|---|---|
| 2026-09-23 | **Avisos también en la tienda** (`templates/_avisos.html`, compartido con el panel): verde con «Deshacer» y rojo con «Ver dónde me equivoqué» en Mi cuenta, login y registro (`error_en_formulario(..., 'pagina', campo=...)`). Login exitoso = solo «Inicio de sesión exitoso». Foto de perfil con «Deshacer» (se conserva la foto anterior; `podar_avatares` deja solo actual + anterior) y perfil admin/cliente con «Deshacer» de datos. Se quitaron los avisos sueltos de cada plantilla (ya no salen mensajes dobles). | `git revert` |
| 2026-09-27 | **Menú real** (`migrations/016_menu_real.sql`): se reemplazó el menú ficticio por el del negocio real, verificado en su plataforma de pedidos en línea (13 categorías, 60 productos, precios exactos). Nueva **personalización de hamburguesas** en 3 grupos: Salsas (obligatorio, mínimo 1), Papas (obligatorio, una sola), Agregados (opcional, con precio) — reemplaza el viejo esquema de "Cremas" reutilizando el mismo mecanismo de BD (`detalleCremas`), solo con categorías nuevas y validación de grupos obligatorios en `detalle-producto.js`. `CATEGORIAS_PERSONALIZACION` (`model/Producto.py`) centraliza qué categorías son ocultas (no aparecen en carta, sugeridos, búsqueda ni conteos). También: dirección y horario reales en `negocio.py` (`SEDE`, `horario_dia`), redes sociales reales (`negocio.REDES`) en el footer de la tienda. Migración con `INSERT ... ON DUPLICATE KEY UPDATE` para no chocar con filas sueltas de pruebas anteriores. | `git revert` + revertir la migración |
| 2026-09-23 | **Foto de perfil** (`migrations/015_foto_perfil.sql`, `usuario.fotoPerfil`): el cliente (`/mi-cuenta`) y el personal del panel (`/admin/perfil`) suben, cambian o quitan su foto; se recorta al centro en cuadrado 320×320 JPEG (`subidas.guardar_avatar`, máx. 5 MB, valida que sea imagen real) y se guarda en `static/img/perfiles/` (gitignored). Aparece junto al nombre en la barra de la tienda, en la barra superior y el pie del menú del panel (`templates/_avatar.html`, contexto global `mi_perfil`); sin foto se ven las iniciales. | `git revert` + revertir la migración |
| 2026-09-22 | **Menú del panel por módulos**: solo pantallas de destino (los filtros y botones viven dentro de cada pantalla; un módulo con una sola pantalla no lleva submenú). Resumen · Pedidos · **Gestión** (Productos, Categorías) · **Ventas y caja** (Ventas, Pagos) · Usuarios (solo superusuario) · Mi perfil. Cada ítem trae en su tooltip las RS que cubre; el módulo activo se abre solo. Barra contraída: el módulo se abre como ventana flotante. Arreglo: en pantallas angostas el modo contraído ya no pisa a la barra abierta. La ruta de navegación (migas) usa los mismos nombres. | `git revert` |
| 2026-09-22 | **Menú lateral del panel contraíble**: la hamburguesa de la marca (`data-adm-toggle`) contrae la barra a solo íconos (64 px) y la vuelve a expandir; la elección se recuerda (`localStorage` `musa.adm.menu`). En pantallas angostas la barra parte contraída y se abre encima del contenido. `static/js/admin-menu.js` + reglas `html.adm-colapsado` / `html.adm-abierto` en el CSS. | `git revert` |
| 2026-09-22 | **Ruta de navegación (migas)**: componente `templates/_migas.html` + CSS `.migas*`. Cada pantalla declara `{% set migas = [(texto, url, icono?), ...] %}` tras su `{% block title %}`; la base pone la raíz (Inicio/Panel). Tienda: bajo el navbar; panel: en la barra superior. En móvil colapsa a «…». Reemplaza las migas simples viejas (`.carta-breadcrumb`, ya sin uso). | `git revert` |
| 2026-09-21 | **RS 9.1/9.2 · Pagos** (`/admin/pagos`, `model/Pago.py`): resumen por medio (efectivo/Yape/Plin/tarjeta), listado con filtros en vivo y período (hoy/ayer/7 días/mes/todo); las anuladas se muestran pero no suman. **RS 2.2**: el administrador ahora corrige DNI, nombres, apellidos, correo y teléfono de una cuenta (con «Deshacer»); la contraseña sigue siendo solo del dueño. | `git revert` |
| 2026-09-21 | **RS 14 · Anular ventas** (`migrations/014_anulacion_ventas.sql`): el comprobante se conserva marcado como anulado (motivo obligatorio, quién y cuándo), sale de KPIs/gráficos/Resumen, opción de devolver stock, «Deshacer anulación», sello en detalle/PDF/vista del cliente y filtro Vigentes/Anuladas. Además: ventas con gráficos por mes/año elegibles y filtros en vivo, avisos con «Deshacer»/«Ver dónde», cobro con 3 opciones (boleta simple / con DNI / factura), arreglo del no-show tras medianoche. | `git revert` + revertir la migración |
...
## 2026-09-21 - Menú real, personalización, sede/redes reales y fotos de producto
- Investigada la plataforma de pedidos real del negocio (`las-musas-burger.ola.click`, ya renombrado "Las de Siempre Burger"): confirma que es el mismo negocio detrás del proyecto. El usuario aportó capturas con precios exactos de cada categoría y del sistema de personalización (salsas/papas/agregados).
- Reemplazado el menú ficticio (6 categorías/20 productos) por el real: **13 categorías, 60 productos** (7 categorías de hamburguesas + Platos Especiales + Combos + Bebidas + 3 categorías ocultas de personalización: Salsas, Papas, Agregados). Generado programáticamente para evitar errores de transcripción.
- Generalizada la vieja categoría única "Cremas" a tres grupos de personalización (`CATEGORIAS_PERSONALIZACION` en `model/Producto.py`): Salsas (obligatorio, mínimo 1), Papas (obligatorio, una sola), Agregados (opcional, de pago). Reutiliza la tabla `detalleCremas` existente sin cambios de esquema; nueva validación obligatoria en `detalle-producto.js` antes de agregar al carrito.
- Corregido un bug preexistente: los productos de las categorías ocultas se colaban en el conteo/búsqueda/autocompletado de la carta pública (`controllers/cliente.py: carta()`); ahora se excluyen explícitamente.
- Actualizados en `negocio.py`: dirección real (Av. José Balta, Chiclayo 14008, Perú), horario real (lunes a sábado 18:00–23:30, domingo 9:00–23:00) y nuevo dict `REDES` (WhatsApp/Instagram/TikTok reales) inyectado por contexto a Jinja; el footer (`client/base.html`) ya no usa `href="#"`.
- Migración `016_menu_real.sql` (idempotente, `INSERT ... ON DUPLICATE KEY UPDATE`, robusta ante ids sueltos que ya existían en la BD real) y `017_fotos_menu.sql` con las fotos reales de 17 productos que el usuario colocó en `static/img/`, renombradas y movidas a `static/img/menu/` (carpeta sí versionada, a diferencia de `productos/`, `categorias/`, `perfiles/`). Ambas migraciones aplicadas a la `db_musuas` local con respaldo previo (`db_musuas_pre017.sql`).
- Actualizados ~10 archivos de tests que dependían de nombres/precios/ids del menú ficticio; nuevo `tests/test_menu_real.py` cubre categorías ocultas, conteo de opciones de personalización por producto y un pedido end-to-end con salsa+papas+agregado pagado.
